...
**Als Gerüst (Kopie von `en.json`, noch unübersetzt) liegen bereit:** `es.json`
(Spanisch), `fr.json` (Französisch), `it.json` (Italienisch), `ja.json` (Japanisch).
Für jede andere Sprache legst du analog eine `<code>.json` als Kopie von `en.json` an.
